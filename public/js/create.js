...
function toggleCorrect(questionIndex, optionIndex) {
    ...
    if (!isChecked && currentCorrect >= 1) {
        showToast('Chỉ được chọn 1 đáp án đúng', 'error');
        renderQuestions();
        return;
    }
...
async function saveQuiz() {
    ...
    for (let i = 0; i < questions.length; i++) {
        ...
        if (!q.question.trim()) {
            showToast(`Câu hỏi ${i + 1}: Vui lòng nhập nội dung câu hỏi`, 'error');
            return;
        }
        
        if (q.options.length < 2) {
            showToast(`Câu hỏi ${i + 1}: Cần ít nhất 2 đáp án`, 'error');
            return;
        }
        
        for (let j = 0; j < q.options.length; j++) {
            if (!q.options[j].text.trim()) {
                showToast(`Câu hỏi ${i + 1}: Đáp án ${String.fromCharCode(65 + j)} không được để trống`, 'error');
                return;
            }
        }

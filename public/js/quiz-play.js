...
function displayQuestion() {
    ...
    q.options.forEach((opt, idx) => {
        ...
        if (userAnswer !== undefined) {
            if (userAnswer.correctIndex === idx) {
                classes += ' correct';
            }
    ...
    document.getElementById('quizArea').innerHTML = `
        <div class="question-card">
            ...
            <div class="quiz-navigation">
                <button class="nav-button prev" onclick="prevQuestion()" ${prevDisabled}>
                    ← Quay lại
                </button>
                <button class="nav-button next" onclick="nextQuestion()" ${nextDisabled}>
                    Tiếp tục →
                </button>
            </div>
        </div>
...
async function checkAnswer(selected) {
    const q = quiz.questions[currentQuestion];
    let userAnswer = userAnswers[currentQuestion] || { wrongAttempts: [] };
...
async function showResults() {
    ...
    const avgTime = validTimes.length > 0 ? 
        Math.floor(validTimes.reduce((a, b) => a + b, 0) / validTimes.length) : 0;
